...
standartized_csv_file <- read_csv(csv_file, col_types=cols(
  name = col_character(),
...
pb <- tkProgressBar( title = "Convart Standartize Fasta CSV",     # Title of bar
                     ...
                     max = nrow(standartized_csv_file),    # Maximum value of the progress bar
                     width = 300)   


foreach(i = 1:nrow(standartized_csv_file)) %do% {
  
  line <- c(sprintf(">%s %s [%s]",  name=standartized_csv_file[i,]$name,
          annotation=str_remove(standartized_csv_file[i,]$annotation, "\\[(.*?)\\]"),
          specie=standartized_csv_file[i,]$specie), standartized_csv_file[i,]$sequence)
  
  write(line,                                            # Write new line to file
