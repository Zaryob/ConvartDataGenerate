...
selectDatabase <- function() {
  ...
  buton2 <-
    tkradiobutton(frame1,
                  ...
                  value = 1,
                  vari = quart)
  tkpack(tklabel(frame1, text = "Select the FASTA source that you process!!!"),
         anchor = "w")
...
selectSpecie <- function(fileName) {
  ## Main Frame
  ...
  tkpack(tklabel(frame1, text = sprintf("Select the specie %s!!!", fileName), anchor = "w"))
  tkpack(buton1, anchor = "w")
  tkpack(buton2, anchor = "w")
  tkpack(buton3, anchor = "w")
  tkpack(buton4, anchor = "w")
  tkpack(buton5, anchor = "w")
  tkpack(buton6, anchor = "w")
  tkpack(buton7, anchor = "w")
  tkpack(buton8, anchor = "w")
  tkpack(buton9, anchor = "w")
...
fasta_files <- tk_choose.files(
  default = "",
  caption = "Select NCBI Fasta File",
  multi = TRUE,
  filter = Filters
)
...
foreach(fasta_file = fasta_files, .combine = 'c') %do% {
  ...
  pb <-
    tkProgressBar(
      title = "Convart Read Fasta",
      # Title of bar
      label = "Importing fasta",
      # Information label
      min = 0,
  ...
  if (dataSource == 0) {
    ...
    foreach(fasta = fastas, .combine = 'c') %do% {
      ...
      fasta_data[nrow(fasta_data) + 1,] <- c(fa_name,
                                             fa_specie,
                                             fa_annot,
                                             fasta[1],
                                             format(Sys.time(), "%a %b %d %X %Y"))   # Append new row
    ...
    fa_specie = selectSpecie(fasta_file)
    foreach(fasta = fastas, .combine = 'c') %do% {
      # Regex patterns
      fa_namePattern = "\\>([\\w\\-.]+)"
      fa_transcriptPattern = "transcript:([\\w\\-.]+)"
      fa_chromosomePattern = "chromosome:((.*?):(.*?):(.*?):(.*?):(.*?) )"
      fa_genePattern = "gene:([\\w\\-.]+) "
      ...
      fa_chromosome = substring(fa_chromosome, 12, nchar(fa_chromosome) -
                                  1)
  ...
  fa_specie <- sub(" ", "_", fa_specie)
  write_csv(
    fasta_data,
    sprintf(
      "%s_%s_fasta_output_data.csv",
      fa_specie,
      dataSourceName
    ),
    col_names = TRUE
